...
public class SavedInventoryTable
{
    private readonly IDbConnection _db;

    private SqlTable inventoriesTable;

    public SavedInventoryTable(IDbConnection db)
    {
        _db = db;
        ...
        sqlCreator.EnsureTableStructure(inventoriesTable);
    }
    ...
    public PlayerData GetPlayerData(TSPlayer player, int acctid)
    {
        ...
        try
        {
            using (var reader = _db.QueryReader($"SELECT * FROM {inventoriesTable.Name} WHERE Account=@0", acctid))
            {
                if (reader.Read())
                {
    ...
    public bool SeedInitialData(UserAccount account)
    {
        ...
        {
            _db.Query(
                $"INSERT INTO {inventoriesTable.Name} (Account, Health, MaxHealth, Mana, MaxMana, Inventory, spawnX, spawnY, questsCompleted) VALUES (@0, @1, @2, @3, @4, @5, @6, @7, @8);",
                account.ID,
    ...
    public bool InsertPlayerData(TSPlayer player, bool fromCommand = false)
    {
        ...
        {
            TShock.Log.ConsoleInfo("{0} is a player name",
        ...
        if (!GetPlayerData(player, player.Account.ID).exists)
        {
            try
            {
                _db.Query(
                    ...
                    TShock.Utils.EncodeColor(player.TPlayer.eyeColor), player.TPlayer.anglerQuestsFinished,
                    player.TPlayer.UsingBiomeTorches ? 1 : 0, player.TPlayer.happyFunTorchTime ? 1 : 0,
                    player.TPlayer.unlockedBiomeTorches ? 1 : 0, player.TPlayer.CurrentLoadoutIndex,
                    player.TPlayer.ateArtisanBread ? 1 : 0, player.TPlayer.usedAegisCrystal ? 1 : 0,
                    player.TPlayer.usedAegisFruit ? 1 : 0, player.TPlayer.usedArcaneCrystal ? 1 : 0,
                    player.TPlayer.usedGalaxyPearl ? 1 : 0, player.TPlayer.usedGummyWorm ? 1 : 0,
                    player.TPlayer.usedAmbrosia ? 1 : 0, player.TPlayer.unlockedSuperCart ? 1 : 0,
        ...
        else
        {
            try
            {
                _db.Query(
                    ...
                    TShock.Utils.EncodeColor(player.TPlayer.skinColor),
                    TShock.Utils.EncodeColor(player.TPlayer.eyeColor), player.TPlayer.anglerQuestsFinished,
                    player.TPlayer.skinVariant, player.TPlayer.extraAccessory ? 1 : 0,
                    player.TPlayer.UsingBiomeTorches ? 1 : 0, player.TPlayer.happyFunTorchTime ? 1 : 0,
                    player.TPlayer.unlockedBiomeTorches ? 1 : 0, player.TPlayer.CurrentLoadoutIndex,
                    player.TPlayer.ateArtisanBread ? 1 : 0, player.TPlayer.usedAegisCrystal ? 1 : 0,
                    player.TPlayer.usedAegisFruit ? 1 : 0, player.TPlayer.usedArcaneCrystal ? 1 : 0,
    ...
    public bool RemovePlayer(int userid)
    {
        try
        {
            _db.Query("DELETE FROM tsCharacter WHERE Account = @0;", userid);
            return true;
        }
        catch (Exception ex)
        {
